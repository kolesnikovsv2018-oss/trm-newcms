...
class NewMenu extends \TRMEngine\Helpers\TRMState
{
  ...
  public $Link;
  /**
   * @var int -  номер узла = номеру ID пункта меню из базы, 
  ...
  public function addChildrenFromRow($row)
  {
    ...
    $tmp->FirstMenuId = $tmp->Numb ? $this->FirstMenuId . "_" . $tmp->Numb : "";
   ...
   * @return $this
   */
  public function getMenuFromDB(TRMMySqlObject $DBO, $position = "top", $present = 1)
  {
  ...
  public function __toString()
  {
    ...
    if (!empty($this->MenuChildrens) && ($tcount = count($this->MenuChildrens)) > 0) {
      ...
      // добавляем id для элемента меню верхнего уровня, если указан
      if (!empty($this->FirstMenuId) && $this->MenuParent == null) {
        $ResStr .= " id='{$this->FirstMenuId}'";
      }

      if (!empty($this->FirstMenuClass)) {
        $ResStr .= " class='{$this->FirstMenuClass}'";
      }
      ...
      $ResStr .= "<li><div>{$LinkString}</div></li>";
    }
    return $ResStr;
  }
} // TRMMenu
