...
class NewPrestigeProducts
{
  ...
  /**
   * @return int|null - ID группы, для которой выбираются популярные товары
   */
  ...
  /**
   * @return int|null - ID производителя, для которого собираются популярные товары
   */
  ...
  /**
   * @param int|null $VendorId - устанавливает ID-производителя, выбираются только его товары
   * @return void
   */
  ...
  public function getPrestigeProducts($Limit = 10, $PrestigeLimit = 0)
  {
    ...
    if ($this->VendorId) {
      ...
      $this->ProducstRepository->addCondition("table1", "vendor", $this->VendorId);
    }
