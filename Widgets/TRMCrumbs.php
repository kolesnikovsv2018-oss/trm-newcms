...
class TRMCrumbs
{
  ...
  public function getOrderedItems(): array
  {
    ...
    $orderedCrumbs = array_reverse($this->Crumbs);
    foreach ($orderedCrumbs as $crumb) {
      $url = $this->URLPrefix;
      if (!empty($crumb[$this->URLField])) {
        $url .= '/' . ltrim($crumb[$this->URLField], '\\/');
        $url .= $this->URLPostfix;
      }

      $items[] = array(
        'name' => $crumb[$this->TitleField],
        'url' => $url ?: '/',
      );
    }
  ...
  public function __toString()
  {
    ...
    $pos = 1;
    if (!empty($this->FirstLink) && !empty($this->FirstTitle)) {
      $str .= "<span itemscope itemprop=\"itemListElement\" itemtype=\"https://schema.org/ListItem\">";
      $str .= "<a href=\"" . $this->FirstLink . "\" itemprop=\"item\">";
      $str .= "<span itemprop=\"name\">" . $this->FirstTitle . "</span>";
      $str .= "</a>" . $this->Separator;
      $str .= "<meta itemprop=\"position\" content=\"$pos\" />";
      $str .= "</span>";
      $pos++;
    }
    ...
    for ($i = $Start; $i > -1; $i--) {
      ...
      $str .= "\" >"
        . "<span itemprop=\"name\">" . $this->Crumbs[$i][$this->TitleField]  . "</span>"
        . "</a>";
      $str .= "<meta itemprop=\"position\" content=\"$pos\" />";
      $str .= "</span>";
      $pos++;
   ...
   * @return boolean
   */
  static public function getParents(TRMMySqlObject $DBO, $id, TRMCrumbs &$crumb, $next = TRUE)
  {
    $query = "SELECT "
      . "{$crumb->IdField}";
    ...
    if ($row) {
      $crumb->Crumbs[] = $row;
      if ($next && !empty($crumb->ParentField)) {
        self::getParents($DBO, $row[$crumb->ParentField], $crumb);
      }
    }
    return true;
  }
