...
{
  /**
   * @var array - массив соответсвия внутренного немера валют, сокращения, кода на сайте центробанка - cbr.ru,
   * а так же строки с возможным сокращением
   */
  ...
  public static function setConfig1($filename)
  {
    if (!is_file($filename)) {
      ...
      echo __METHOD__ . " Файл с настройками получить на удалось [{$filename}]!";
      return false;
    }
    ...
    if (!is_array(self::$ConfigArray) || empty(self::$ConfigArray)) {
      ...
      echo __METHOD__ . " Файл конфигурации вернул неверный формат данных [{$filename}]!";
      return false;
    }
  ...
  public static function createCourseFile($coursefilename)
  {
    ...
    foreach (static::$Valutas as $index => $val) {
      if (!isset($val["CBRCode"])) {
        $f->addToBuffer("1\n");
      } else {
        $f->addToBuffer(static::getXMLCourseFromCB($val["CBRCode"]) . "\n");
      }
    }
    $f->addToBuffer(time() . "\n");
    $f->putBufferTo();
