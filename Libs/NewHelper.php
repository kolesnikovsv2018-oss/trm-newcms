...
class NewHelper
{
   ...
   */
  public static function recursivePrice(TRMMySqlObject $DBO, $startrow)
  {
    //получаем все товары, 
...
AND `ID_Complect` =" . $startrow["ID_Price"];
   ...
   */
  public static function getAllChildsArray(TRMMySqlObject $DBO, $StartId, $TableName, $IdFieldName, $ParentFieldName, $OrderFieldName = null, $PresentFieldName = null, $first = true)
  {
    ...
    if ($first === true) {
      $allgroups->mergeDataArray(explode(",", $StartId));
    }
    ...
    while ($row1 = $result1->fetch_array(MYSQLI_ASSOC)) {
      ...
      if ($allgroups->inArray($row1[$IdFieldName])) {
        continue;
      }
...
//*************** НАДО ПОДУМАТЬ ГДЕ РЕАЛИЗОВЫВАТЬ ЭТОТ МЕТОД ************************
   ...
   */
  public static function createLinkRows(TRMMySqlObject $DBO, $CountOfVisualElement, $CurrentId, $groupnumber = null)
  {
