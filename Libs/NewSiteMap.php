...
class NewSiteMap extends TRMState
{
  ...
  public function __construct(TRMMySqlObject $DBO, array $Config = array())
  {
    ...
    if (key_exists("HTTP_X_REQUEST_SCHEME", $_SERVER)) {
      ...
      self::$Protocol = TRMLib::sanitize_string($_SERVER["HTTP_X_REQUEST_SCHEME"] ?? '');
      ...
      self::$Protocol = TRMLib::sanitize_string($_SERVER["REQUEST_SCHEME"] ?? '');
    } else {
  ...
  public function generateSiteMapsMain($priority = "0.8")
  {
    ...
    if (!$result || $result->num_rows <= 0) {
      ...
      $this->SiteMaps["Main"] .= '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">';
    ...
    while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
      ...
      if (strpos($row["MenuLink"], "news") || strpos($row["MenuLink"], "articles")) {
        $changefreq = "daily";
      } else {
        $changefreq = "weekly";
      }
  ...
  public function generateSiteMapsArticles($priority = "0.5", $changefreq = "yearly")
  {
    ...
    if (!$result || $result->num_rows <= 0) {
      ...
      $this->SiteMaps["Articles"] .= '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">';
  ...
  public function generateSiteMapsGroup($priority = "0.6", $changefreq = "yearly")
  {
    ...
    if (!$result || $result->num_rows <= 0) {
      ...
      $this->SiteMaps["Group"] .= '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">';
  ...
  public function generateSiteMapsPrice()
  {
    ...
    if (!$result || $result->num_rows <= 0) {
      ...
      $this->SiteMaps["Price"] .= '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">';
  ...
  {
    if (!$this->generateSiteMapsMain()) {
      return false;
    }

    if (!$this->generateSiteMapsArticles()) {
      return false;
    }

    if (!$this->generateSiteMapsGroup()) {
      return false;
    }

    if (!$this->generateSiteMapsPrice()) {
      return false;
    }
    ...
    for (reset($this->SiteMaps); ($key = key($this->SiteMaps)); next($this->SiteMaps)) {
      if (isset($this->SiteMaps[$key]) && strlen($this->SiteMaps[$key]) > 0) {
        ...
        if (false === file_put_contents(ROOT . "/" . $filename, $this->SiteMaps[$key])) {
          $this->addStateString("Не могу записать файл SiteMap для " . $key);
          return false;
        }
    ...
    if (false === file_put_contents(ROOT . "/" . $filename, $this->SiteMapIndex)) {
      $this->addStateString("Не могу записать файл SiteMapIndex.XML");
      return false;
    }
