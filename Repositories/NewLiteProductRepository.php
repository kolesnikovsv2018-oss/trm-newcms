...
class NewLiteProductRepository extends NewIdTranslitRepository
{
    ...
    "table1" => array(
      ...
      TRMDataMapper::FIELDS_INDEX => array(
        ...
        "unit" => array(
          ...
          ), // из unit будет выбрана запись (только одна запись из unit для каждой из table1 так как ID_unit - уникален!!!), для которой `unit`.`ID_unit` === `table1`.`unit`
        ),
      )
    ),
    "unit" => array(
  ...
  public function update(TRMDataObjectInterface $DataObject)
  {
    ...
    $DataObject->setData("table1", "ModifyDate", date('Y-m-d H:i:s'));
   ...
   * и наценки
   */
  static protected function calculateProductPrice(TRMMySqlObject $DBO, NewLiteProduct $LiteProduct)
  {
    try {
   ...
   */
  public static function getProductsIdFromDB(TRMMySqlObject $DBO, $GroupId, $SubGroupFlag = false, $PresentFlag = true)
  {
